...
- `POST /api/characters` - 创建角色（可选 userId 参数）
...
1. **密码存储**: 当前未实现密码字段，未来添加时应使用哈希加盐存储
2. **邮箱验证**: 建议添加邮箱验证机制
3. **用户名规则**: 建议在应用层添加用户名格式验证
4. **访问控制**: 确保用户只能访问自己的角色数据
