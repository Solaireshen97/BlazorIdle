...
1. **配置参数化** ✅
   - [x] ShopSettings 配置类
   - [x] appsettings.json 配置节
   - [x] 时间周期配置化（每日/每周重置）
   - [x] 分页参数配置化
   - [x] 缓存时间配置化

2. **缓存系统** ✅
   - [x] CachedShopService 装饰器实现
   - [x] 商店定义缓存（5分钟）
   - [x] 商品列表缓存（2分钟）
   - [x] 购买成功后自动失效
   - [x] 可配置启用/禁用
...
3. **性能优化** ✅
   - [x] 批量查询购买计数器（避免 N+1 查询）
   - [x] 提取重置逻辑到独立方法

4. **依赖注入优化** ✅
   - [x] 注册 IMemoryCache
   - [x] 装饰器模式注册 CachedShopService
   - [x] 配置验证和绑定
...
### 2. 缓存策略
- **装饰器模式**：代码解耦，易于维护
- **分级缓存时间**：根据数据变化频率设置不同缓存时间
- **智能失效**：购买成功自动清除相关缓存
- **可配置控制**：通过配置开关控制缓存启用

### 3. 性能优化
- **批量查询**：一次性加载所有购买计数器，减少数据库往返
- **查询优化**：从 O(N) 查询降低到 O(1)
- **内存效率**：使用 Dictionary 快速查找
