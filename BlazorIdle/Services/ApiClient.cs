...
public class ApiClient
{
    ...
    /// 获取购买历史
    /// </summary>
    public Task<BlazorIdle.Shared.Models.Shop.PurchaseHistoryResponse?> GetPurchaseHistoryAsync(string characterId, int page = 1, int pageSize = 20, CancellationToken ct = default)
    {
        SetAuthHeader();
        return _http.GetFromJsonAsync<BlazorIdle.Shared.Models.Shop.PurchaseHistoryResponse>($"/api/shop/history?characterId={characterId}&page={page}&pageSize={pageSize}", ct);
    }

    /// <summary>
    /// 根据ID获取角色信息
    /// </summary>
    public Task<UserCharacterDto?> GetCharacterAsync(Guid characterId, CancellationToken ct = default)
    {
        SetAuthHeader();
        return _http.GetFromJsonAsync<UserCharacterDto>($"/api/characters/{characterId}", ct);
    }
}
