...
### Bug Fix: 副本波次切换时玩家无敌问题 (2025-10) ✅ 已修复

#### 问题描述
- 用户报告：在副本战斗中，第一波战斗玩家正常受伤和死亡，但进入第二波及后续波次后，玩家不再受到怪物攻击，变成无敌状态
- 影响范围：所有多波副本战斗（DungeonSingle 和 DungeonLoop 模式）

#### 根本原因分析
1. **症状**：第二波及后续波次中，怪物不会攻击玩家
2. **定位过程**：
   - 检查 `EnemyAttackEvent.Execute()` 中的条件判断
   - 发现 `Enemy.CanAct()` 返回 false
   - 追踪到 `EnemyCombatant` 引用的 `Encounter` 是上一波已死亡的怪物
3. **根本原因**：
   - 波次切换时，`TryPerformPendingSpawn()` 调用 `Context.ResetEncounterGroup(_pendingNextGroup)` 更新了 `EncounterGroup`
   - **但没有重新初始化 `Context.EnemyCombatants` 列表**
   - 旧的 `EnemyCombatant` 实例仍然引用第一波已死亡的怪物
   - 因此 `Enemy.CanAct()` 检查失败，导致不会调度新的攻击事件

#### 修复方案
在 `BattleEngine.TryPerformPendingSpawn()` 方法中，波次切换时：
1. 清空旧的 `Context.EnemyCombatants` 列表
2. 重新调用 `InitializeEnemyAttacks(Context.EncounterGroup!)` 为新波次创建新的怪物战斗单位
3. 重新调用 `InitializeEnemySkills(Context.EncounterGroup!)` 初始化新波次的怪物技能

#### 修改的文件
- **BlazorIdle.Server/Domain/Combat/Engine/BattleEngine.cs**
  - 在 `TryPerformPendingSpawn()` 方法中添加 3 行代码：
    ```csharp
...
#### 测试验证
- **新增测试**：`DungeonMultiWavePlayerDamageTests.cs`
  - 测试 `DungeonBattle_EnemyCombatants_ReinitializedOnWaveTransition()`
  - 验证波次切换时怪物战斗单位被正确重新初始化
  - 验证新波次的怪物能够正常攻击玩家
- **回归测试**：所有现有测试通过（51/51 战斗相关测试）

#### 影响分析
- ✅ 修复了副本多波战斗中玩家无敌的bug
- ✅ 确保每个波次的怪物都有独立的 `EnemyCombatant` 实例
- ✅ 不影响单怪战斗和持续模式战斗
- ✅ 向后兼容，不破坏现有功能
- ✅ 代码修改最小化，仅3行核心修复代码
