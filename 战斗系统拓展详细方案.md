...
#### 任务清单
- [x] **P4.1**: 扩展 EnemyDefinition ✅ 已完成 (2025-01)
  - 添加 `int BaseDamage`
  - 添加 `DamageType AttackDamageType`
  - 添加 `double AttackIntervalSeconds`（攻击间隔）
  
- [x] **P4.2**: 初始化怪物攻击轨道 ✅ 已完成 (2025-01)
  - 在 BattleEngine 创建时，为每个怪物创建 AttackTrack
  - 调度第一个 EnemyAttackEvent
  - 新波次刷新时清理旧轨道并重新初始化
  
- [x] **P4.3**: 创建 EnemyAttackEvent ✅ 已完成 (2025-01)
  - 类似 AttackTickEvent，但作用于怪物
  - 使用 TargetSelector 选择玩家目标（当前单人，直接选玩家）
  - 计算伤害（基础值，不考虑暴击/穿透，保持简单）
  - 调用 `Player.ReceiveDamage`
  - 检测玩家死亡并调度 PlayerDeathEvent
  
- [x] **P4.4**: 怪物无可攻击目标时暂停 ✅ 已完成 (2025-01)
  - 检查是否有存活玩家 (`Player.CanBeTargeted()`)
  - 如果所有玩家死亡，暂停怪物攻击 Track（使用 Pause/Resume 机制）
  - 玩家复活时，恢复怪物攻击 Track
  - 在 TrackState 中实现 Pause() 和 Resume() 方法
  
- [x] **P4.5**: 伤害平衡调整 ✅ 已完成 (2025-01)
  - 配置怪物基础伤害，确保玩家不会瞬间死亡
  - dummy: 5伤害/2.0s = 2.5 DPS (训练用)
  - tank: 15伤害/2.0s = 7.5 DPS (坦克型)
  - magebane: 12伤害/2.0s = 6.0 DPS (法术型)
  - paper: 8伤害/1.5s = 5.3 DPS (快速型)
  
- [x] **P4.6**: 单元测试 ✅ 已完成 (2025-01)
  - ✅ 测试 EnemyDefinition 扩展属性
  - ✅ 测试怪物可以攻击玩家
  - ✅ 测试玩家受伤、HP 减少
  - ✅ 测试玩家死亡后怪物暂停攻击
  - ✅ 测试玩家复活后怪物恢复攻击
  - ✅ 测试 TrackState 暂停/恢复机制
  - ✅ 测试 BattleEngine 集成（15个测试用例，全部通过）
