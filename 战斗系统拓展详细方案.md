...
- [ ] **P3.7**: 更新 BattleEngine 结束条件
  - ⚠️ 玩家死亡不立即结束战斗（现有逻辑已支持，无需额外修改）
  - ⚠️ 等待复活后继续战斗（现有逻辑已支持）
  - ⚠️ 仅当时长耗尽或怪物全灭时结束（现有逻辑已符合）
  
- [x] **P3.8**: 配置复活时长
  - ✅ 默认 10 秒（在 PlayerCombatant 中实现）
  - ✅ 支持副本配置覆盖（通过 ReviveDurationSeconds 属性）
...
#### 交付物
- ✅ 更新 `PlayerCombatant.cs` - 实现实际伤害处理和死亡/复活逻辑
- ✅ `PlayerDeathEvent.cs` - 新增玩家死亡事件处理
- ✅ `PlayerReviveEvent.cs` - 新增玩家复活事件处理
- ✅ 更新 `AttackTickEvent.cs` - 添加玩家存活状态检查
- ✅ 更新 `SpecialPulseEvent.cs` - 添加玩家存活状态检查
- ✅ 更新 `StepBattleCoordinator.cs` (StepBattleStatusDto) - 添加玩家 HP 和复活信息
- ✅ `PlayerDeathReviveTests.cs` - 11 个单元测试验证死亡复活流程
- ✅ 更新 `CombatantTests.cs` - 修改测试以反映 Phase 3 行为

#### 验收标准
- ✅ 玩家可以死亡并自动复活（已通过 11 个单元测试验证）
- ✅ 死亡时攻击暂停，复活后恢复（已验证）
- ✅ 战斗不因玩家死亡而结束（现有逻辑已支持）
- ✅ UI 正确显示玩家 HP 和复活倒计时（已添加到 DTO）

#### 完成状态
**状态**: ✅ 已完成 (2025-01)
**实施人员**: Solaireshen97 + GitHub Copilot
**测试通过**: 25/25 (14 Combatant + 11 DeathRevive)
