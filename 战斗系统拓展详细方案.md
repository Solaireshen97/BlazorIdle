...
### Phase 6: 强化型地下城预留（Week 14-15）✅ 已完成 (2025-10)
...
- [x] **P6.2**: 更新 PlayerCombatant
  ...
  - ✅ 由副本配置控制，在 ReceiveDamage 中检查
  
- [x] **P6.3**: 更新 PlayerDeathEvent
  - ✅ 检查 `AutoReviveAllowed`
  - ✅ 如果禁用，不调度 PlayerReviveEvent
  - ✅ 记录 `player_death_no_revive` 标签
  
- [x] **P6.4**: 实现副本重置机制
  - ✅ 标记战斗失败（设置 Completed = true）
  - ✅ 添加 ResetTriggered 和 ResetTime 属性
  - ✅ 在 BattleEngine 中添加 CheckPlayerDeathReset 方法
  - ✅ 记录 `dungeon_reset_on_death` 标签
  
- [x] **P6.5**: 强化掉落倍率
  - ✅ EconomyCalculator 中已有 DropChanceMultiplier 支持
  - ✅ EnhancedDropMultiplier 可结合使用
  
- [ ] **P6.6**: UI 提示（未实施，预留前端功能）
  - 显示副本是否允许复活
  - 死亡时显示重置提示
...
#### 交付物
- ✅ 更新 `DungeonDefinition.cs` (+9行)
- ✅ 更新 `DungeonEncounterProvider.cs` (+3行)
- ✅ 更新 `PlayerCombatant.cs` (+6行)
- ✅ 更新 `PlayerDeathEvent.cs` (+16行)
- ✅ 更新 `BattleEngine.cs` (+54行)
- ✅ `EnhancedDungeonTests.cs`（新建，449行，14个测试）
